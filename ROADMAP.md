...
- **Cobertura de testes**: 1107 testes unitários (39 novos para o resolver de publicações HTML Xtream + fan-out Telegram + sanitização), 0 warnings em `dotnet build m3uCrawler.sln --configuration Release` (snapshot em 2026-09-09; warning CS8629 em `ChannelMatcher.cs:267` é pré-existente do iter anterior e está fora do escopo desta iteração).
