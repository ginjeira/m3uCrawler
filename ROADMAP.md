...
- **Cobertura de testes**: 1017 testes unitários, 0 warnings em `dotnet build m3uCrawler.sln --configuration Release` (snapshot em 2026-09-03).
...
- **Fronteira Classification → Matching** (esta iteração): `ContentClassifier.Classify(title, sourceGroup)` corre **antes** do bucket de canais. Apenas `ChannelKind.Channel` produz uma `ChannelDecision`. Bundles, VOD, LiveCam, Placeholder e Unknown são contabilizados como `ClassifiedExclusion` (sem URL nem credenciais). A entrada `PT - NO EVENT` (8 ocorrências na fixture real de produção) deixa de ser promovida a `NewChannel`. O dicionário `ChannelCategoryLookup` continua a ser a fonte primária de verdade — só identidades explicitamente mapeadas são canais. Endpoint `GET /api/classification-summary` no dashboard expõe as contagens e uma amostra das exclusões.
