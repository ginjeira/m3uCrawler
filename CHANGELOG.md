...
- **PHASE 13 — Wave 13-5 (2026-09-18): preview/dry-run read-only da selecção de fontes + métricas.**
  ...
  - **Métricas agregadas:** `channelsProcessed`, `channelsWithSources`, `candidateStreamCount`, `selectedStreamCount`, `rejectedStreamCount`, `unmatchedStreamCount`, `ambiguousStreamCount`, `channelsAtChannelLimit`, `channelLimitRejectionCount`, `providerLimitRejectionCount`, `distinctProviderSelectionCount`, `fillSelectionCount`, `fallbackDisabledRejectionCount`, `sourceDisabledRejectionCount`, `rejectionCounts` e `providerDistribution` (`provider`, `selectedCount`, `channelCount`).
  - **Endpoint/UI:** `GET /api/catalog/source-selection-policies/preview[?channelKey=<CanonicalChannel.Key>]` — GET-only (outros métodos `405`), sob o gate de autenticação/CSRF existente e o gate de catálogo (`503` "Catálogo não inicializado."). Filtro exacto ordinal por `CanonicalChannel.Key`; sem correspondência devolve `applied=false` com métricas zeradas. Cartão "Preview / Dry-Run" e `loadSourceSelectionPreview()` no separador existente.
  ...
  - **Limitações:** o input é o **catálogo**, não um conjunto de descoberta Telegram ao vivo; a disponibilidade do catálogo é usada como proxy de funcionamento; a ambiguidade é **agregada** (`ambiguousStreamCount`) e as entradas não correspondidas têm motivo `unmatched` (não são rotuladas individualmente ambíguas); `fillSelectionCount` é o proxy da Fase B (motivo `fill`); canais sem `ChannelSource` contam em `channelsProcessed` mas não aparecem em `channels`; apenas os dois pontos de publicação Telegram aplicam selecção em produção.
  - **Garantia de não-mutação:** o dry-run corre o mesmo estágio da produção e não faz publicação, alteração de catálogo/ownership/Dispatcharr nem cria estado persistido (o loader read-only remove a única escrita lazy desse caminho).
