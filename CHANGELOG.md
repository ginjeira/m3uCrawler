...
### ✨ Adicionado
- **PHASE 13 — Wave 13-5 (2026-09-18): preview/dry-run read-only da selecção de fontes + métricas.**
  - **Preview read-only:** novo `SourceSelectionPreviewService.PreviewAsync(string? canonicalChannelKey = null, CancellationToken = default)`. Lê o catálogo (`ListCanonicalChannelsAsync` + `ListChannelSourcesAsync`), sintetiza **uma `M3uStream` por `ChannelSourceEntity`** (`Url` = `StreamUrl` sanitizada já armazenada; `IsWorking = Availability not in {Dead, Unreachable}`) e corre o **mesmo** `SourceSelectionStage`/`ChannelSourceSelector` usado em produção — **sem algoritmo duplicado**, sem publicação, sem escrita de ficheiros, sem mutação de catálogo/ownership/Dispatcharr e sem estado persistente.
  - **Carregamento read-only da política:** `CatalogResolver.GetGlobalSourceSelectionPolicyAsync` (nunca insere a linha global; `AsNoTracking`) + `SourceSelectionPolicyResolver.LoadEffectivePoliciesReadOnlyAsync` (fallback para `SourceSelectionDefaults.DefaultPolicy` com `HasExplicitGlobal=false` quando a linha não existe). `SourceSelectionPolicySet.HasExplicitGlobal`/`HasOverride` rotulam o âmbito efectivo (`override`/`global`/`default`).
  - **Resultado por canal (aditivo):** `SourceSelectionStageResult.Channels` + `SourceSelectionChannelResult`. Modelos de preview: `SourceSelectionPreviewResult`, `SourceSelectionPreviewChannel`, `SourceSelectionPreviewCandidate`, `SourceSelectionPreviewUnmatched`, `SourceSelectionPreviewMetrics`, `SourceSelectionPreviewProviderStat`, `SourceSelectionPreviewSourceInfo` e `SourceSelectionPreviewDecisions` (`selected`/`rejected`).
  - **Métricas agregadas (corrigidas):** `channelsProcessed`, `channelsWithSources`, `candidateStreamCount`, `selectedStreamCount`, `rejectedStreamCount`, `unmatchedStreamCount` (só não-ambíguas), `ambiguousStreamCount` (só ambíguas), `totalUnmatchedStreamCount` (soma das duas), `channelsAtChannelLimit`, `channelLimitRejectionCount`, `providerLimitRejectionCount`, `diversitySelectionCount`, `distinctProviderCount`, `fillSelectionCount`, `fallbackDisabledRejectionCount`, `sourceDisabledRejectionCount`, `rejectionCounts` e `providerDistribution` (`provider`, `selectedCount`, `channelCount`). O antigo `distinctProviderSelectionCount` era enganador e foi renomeado para `diversitySelectionCount` (nº de selecções com motivo `diversity`); o novo `distinctProviderCount` é o nº de fornecedores distintos com ≥1 selecção (= `providerDistribution.Count`).
  - **Endpoint/UI (corrigido):** `GET /api/catalog/source-selection-policies/preview[?channelKey=<CanonicalChannel.Key>]` — GET-only (outros métodos `405`), sob o gate de autenticação; `GET` não exige CSRF; e o gate de catálogo (`503` "Catálogo não inicializado."). O match de `channelKey` é **exacto e case-sensitive** (`Ordinal`); chave desconhecida devolve **HTTP 200** com `applied=false`, `status="channel-not-found"` e `channelsProcessed=0`, e `source.channelKeyFilter` é sanitizado antes da emissão. A rota é envolvida em `try/catch`: falha inesperada devolve `500 {"error":"preview-failed"}` e fecha sempre o response (o cliente não fica pendurado). Cartão "Preview / Dry-Run" e `loadSourceSelectionPreview()` no separador existente.
  - **Contrato de `status` (MAJOR-1):** novo campo top-level `status` (`SourceSelectionPreviewStatuses`) que desambigua os quatro casos antes colapsados em `applied=false`: `channel-not-found` (o filtro `channelKey` não corresponde a nenhum canal canónico; `applied=false`, `channelsProcessed=0`), `no-channels` (o catálogo não tem canais canónicos), `no-input` (há canais no âmbito mas nenhum tem `ChannelSource`; `applied=false` com `channelsProcessed>0` é **válido e esperado**) e `applied` (a selecção correu). Precedência: `channel-not-found` → `no-channels` → `no-input` → `applied`. `channelsProcessed` mantém-se "canais canónicos no âmbito" (não foi zerado) e `applied` **não** foi redefinido. A UI usa `status` para distinguir as mensagens de não-aplicado.
  - **Nota de âmbito (MAJOR-2):** `WebDashboardService.WriteJsonAsync` **não** foi alterado. A rota de preview passa o HTTP status explicitamente (`503` sem catálogo, `500` no `catch`) e fecha sempre o response. O default pré-existente do helper partilhado — escrever `OK` quando nenhum status explícito é passado — é dívida transversal **pré-existente**, **fora de âmbito** da Wave 13-5, e **não** foi estruturalmente corrigido. Não se afirma qualquer correcção global do helper.
  - **Sanitização:** todas as URLs emitidas passam `CredentialSanitizer.SanitizeUrl` (defensivo, mesmo que o catálogo já guarde URLs sanitizadas); `Unmatched.Title` passa `SanitizeText`; enums achatados via `ToString()`; origem é o rótulo curto `catalog`, sem caminhos de filesystem. Nenhuma credencial no output.
  - **Contrato de contagens (corrigido):** `unmatched` e `ambiguous` são disjuntos por construção — `metrics.unmatchedStreamCount` conta só as não-ambíguas, `metrics.ambiguousStreamCount` só as ambíguas e `metrics.totalUnmatchedStreamCount` é a soma. O endpoint emite `ambiguous[]` (mesma forma de `unmatched[]`) além de `unmatched[]`; `SourceSelectionStageResult` ganhou a lista aditiva `AmbiguousStreams` sem alterar a semântica de produção de `Unmatched` (continua a incluir as ambíguas). Invariantes: `inputStreamCount == candidateStreamCount + unmatchedStreamCount + ambiguousStreamCount` e `candidateStreamCount == selectedStreamCount + rejectedStreamCount`.
  - **Limitações:** o input é o **catálogo**, não um conjunto de descoberta Telegram ao vivo; a disponibilidade do catálogo é usada como proxy de funcionamento; `unmatched[]` e `ambiguous[]` são listas top-level disjuntas; `fillSelectionCount` é o proxy da Fase B (motivo `fill`); `channelsProcessed` são os canais canónicos no âmbito (incluindo os sem `ChannelSource`), rotulados na UI como "Canais no âmbito", mas os sem fontes não aparecem em `channels`; apenas os dois pontos de publicação Telegram aplicam selecção em produção.
  - **Limitação de paridade: `ResponseTime`.** O preview alimenta `ResponseTime = ChannelSourceEntity.LastResponseTimeMs`, ou seja, usa o valor **persistido** quando presente; na prática essa coluna só é escrita como `0` no insert e nunca é actualizada (`CatalogResolver.cs:1422`; update `:1392-1404`), pelo que está normalmente a `0`/indisponível e **não** representa o `DurationMs` da probe ao vivo; `ChannelSourceObservationEntity` é append-only, sem flag de sucesso, e só é escrito por um POST controlado pelo cliente (`WebDashboardService.cs:2156`); o pipeline ignora `stream.ResponseTime` (`PipelineIngestionService.cs:250-264`). O valor real em produção é o stopwatch ao vivo `DurationMs` da probe exacta (`M3uTesterService.cs:550,555`). Por isso o preview **não** reproduz a ordenação por `ResponseTimeKey` (`ChannelSourceSelector.cs:128,260-261`) e a sua ordenação por response time **pode divergir** da produção: em empates nas primeiras quatro chaves de ranking, a ordem e o conjunto seleccionado podem diferir da produção. É uma limitação documentada, **não** uma garantia.
  - **Não-mutação (estrutural):** o dry-run corre o mesmo estágio da produção; o caminho de preview não tem writer — não faz publicação, alteração de catálogo/ownership/Dispatcharr nem cria estado persistido (o loader read-only remove a única escrita lazy desse caminho).
  - **Fora de âmbito (13-6 e posteriores):** contrato `MatchPlan` + `DispatcharrSourceSelection`, ownership/cleanup selectivo, teste 100→10, `ProviderDefinition`, `SelectionPolicy` (estratégia de ranking separada), `MinimumValidatedSources` e churn/estabilidade. `RunReport.SourceSelection` mantém-se **inalterado** (só contagens); as métricas ricas são âmbito exclusivo do preview. A **Wave 13-6 permanece não implementada**.
  - **Correcções documentais:** `docs/architecture/dispatcharr-source-selection.md` §10.3 e `m3uCrawler/README.md` passam a documentar o campo `status` (valores, precedência e o caso `no-input`), a usar o wording "gate de autenticação; `GET` não exige CSRF" e a descrever o `ResponseTime` como o valor persistido (normalmente `0`/indisponível, não o `DurationMs` da probe ao vivo, podendo a ordenação divergir da produção). Corrigida a referência da escrita de observação para `WebDashboardService.cs:2156`.
  - Documentado em `docs/architecture/dispatcharr-source-selection.md` §10.3, `docs/IMPLEMENTATION_ROADMAP.md` §32.19/§17 e `m3uCrawler/README.md`.

- **PHASE 13 — Wave 13-4b (2026-09-18): overrides por canal da política de selecção de fontes.**
  - **Identidade:** cada override é chaveado por `CanonicalChannel.Key` (string estável), **nunca** por `CanonicalChannelId`. O `ScopeKey` persistido é `channel:<CanonicalChannelKey>` (`SourceSelectionPolicyScopes`), mantendo o índice único da tabela `source_selection_policies`. **Sem alteração de schema ou migration** — a coluna `ScopeKey` já acomodava o formato.
  - **Substituição completa:** um override por canal é uma política **completa** que substitui a política global por inteiro quando existe (a entidade não tem representação de "campo não definido"); **não** há merge campo a campo. Resolução: override por canal → global → defaults (`10/true/null/true`). Overrides sem FK para o canal são inertes se o canal não existir; a identidade sobrevive a apagar/recriar o canal com a mesma `Key`.
  - **Catálogo:** `CatalogResolver` ganha `GetChannelSourceSelectionPolicyAsync`, `UpsertChannelSourceSelectionPolicyAsync`, `DeleteChannelSourceSelectionPolicyAsync` e `ListChannelSourceSelectionPoliciesAsync`; os métodos globais mantêm-se inalterados.
  - **Resolução em lote (sem N+1):** `SourceSelectionPolicyResolver` mantém `ResolveGlobalAsync` e acrescenta `ResolveEffectiveAsync(key)` e `LoadEffectivePoliciesAsync()`, que devolve um `SourceSelectionPolicySet : ISourceSelectionPolicyProvider` (snapshot por execução, **2 queries**, sem cache partilhada entre execuções). Os dois pontos de publicação Telegram usam `LoadEffectivePoliciesAsync` (fallback `SourceSelectionPolicySet.Default` quando o catálogo é nulo).
  - **Estágio:** `SourceSelectionStage` ganha o overload `ApplyAsync(streams, ISourceSelectionPolicyProvider, ct)` e resolve a política efectiva por grupo de canal canónico via `CanonicalChannel.Key`; o overload legado de política única mantém-se, o estágio permanece **sem persistência** e o agrupamento continua por `CanonicalChannelId` (selector/ranking/limites inalterados).
  - **Dashboard:** `GET/POST /api/catalog/source-selection-policies/channels` e `GET/DELETE /api/catalog/source-selection-policies/channels/{key}`, sob o mesmo gate de autenticação/CSRF dos endpoints globais; a UI lista, edita e elimina overrides (texto de ajuda de substituição completa). Endpoints globais inalterados.
  - **Semântica preservada:** `MaxSourcesPerChannel` `0` é válido (o override publica zero fontes para esse canal), `1..N` válido e negativos inválidos; `MaxSourcesPerProvider` `null` = sem limite, com `0`/negativos inválidos.
  - **Testes:** +23 casos (resolver, persistência por canal, endpoints HTTP e integração runtime). Suite serial: 1845 passed / 0 failed / 1 skipped; build Release 0 erros, sem warnings novos. Documentado em `docs/architecture/dispatcharr-source-selection.md` §10.2 e `docs/architecture/phase-13-4-source-selection-policy.md` §0.1.
  - **Fora de âmbito:** preview/dry-run, `ProviderDefinition`, `SelectionPolicy`, `MinimumValidatedSources`, churn/estabilidade, integração explícita no composer/`MatchPlan`/`DispatcharrSyncService` e auditoria administrativa.

- **PHASE 9C.6 (2026-09-18): identidade canónica em runtime — resolução de afinidades Key-autoritativa.**
  - `CatalogResolver.ResolveAsync` resolve afinidades de canal por `AffinityGroup.CanonicalChannelKey` (canal encontrado e habilitado). Se a `Key` existir mas não resolver (inexistente ou desactivada), **não** recua para o `CanonicalChannelId` obsoleto; prossegue para o passo seguinte da cadeia. Linhas legadas com `Key` nula/vazia mantêm o fallback existente por FK/navegação. Uma afinidade **sobrevive** a apagar/recriar o canal com a mesma `Key` (o `Id` técnico muda, a `Key` mantém-se).
  - `CatalogResolver.ListChannelSourcesAsync` passa a incluir `CanonicalChannel` (`.Include(cs => cs.CanonicalChannel)`), expondo `CanonicalChannel.Key` ao caminho de selecção de fontes.
  - **Sem alteração de schema, migration, coluna ou FK:** `CanonicalChannelId` permanece coluna de transição. O `SourceSelectionStage` continua a agrupar por `CanonicalChannelId` (semântica do selector, ranking e limites inalterados). Permanecem dependentes de `Id`: FKs de `channel_aliases`/`channel_sources`/`ordering_items`, `source_priority_policies` por canal, `dispatcharr_channel_ownerships`, `matching_audits`, `MatchPlan` e o agrupamento do `SourceSelectionStage` — **não** é uma migração completa `Id → Key`.
  - **Fora de âmbito:** overrides por canal (Wave 13-4b); fica apenas satisfeito o pré-requisito de identidade para essa wave (Key acessível ao loader de selecção).
  - **Testes:** novo `m3uCrawler.Tests/Phase9C6CanonicalIdentityTests.cs` (6 testes: sobrevivência a delete/recreate com a mesma Key, `Id` nulo + Key válida, Key autoritativa sobre `Id` obsoleto, Key desconhecida sem recuo para `Id` obsoleto, fallback legado sem Key, `ListChannelSourcesAsync` publica a Key). Suite serial: 1822 passed / 0 failed / 1 skipped.
  - Documentado em `docs/architecture/channel-catalog-and-ownership.md` §12 e `docs/architecture/dispatcharr-source-selection.md` §10.
