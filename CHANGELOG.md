...
- **PHASE 13 — Wave 13-5 (2026-09-18): preview/dry-run read-only da selecção de fontes + métricas.**
  ...
  - **Endpoint/UI (corrigido):** `GET /api/catalog/source-selection-policies/preview[?channelKey=<CanonicalChannel.Key>]` — GET-only (outros métodos `405`), sob o gate de autenticação (sessão/token; `GET` não exige CSRF) e o gate de catálogo (`503` "Catálogo não inicializado."). O match de `channelKey` é **exacto e case-sensitive** (`Ordinal`); chave desconhecida devolve **HTTP 200** com `applied=false` e métricas zeradas, e `source.channelKeyFilter` é sanitizado antes da emissão. A rota é envolvida em `try/catch`: falha inesperada devolve `500 {"error":"preview-failed"}` e fecha sempre o response (o cliente não fica pendurado). Cartão "Preview / Dry-Run" e `loadSourceSelectionPreview()` no separador existente.
  ...
  - **Limitação de paridade: `ResponseTime`.** O preview alimenta `ResponseTime = ChannelSourceEntity.LastResponseTimeMs`, mas essa coluna só é escrita como `0` no insert e nunca é actualizada (`CatalogResolver.cs:1422`; update `:1392-1404`); `ChannelSourceObservationEntity` é append-only, sem flag de sucesso, e só é escrito por um POST controlado pelo cliente (`WebDashboardService.cs:2133`); o pipeline ignora `stream.ResponseTime` (`PipelineIngestionService.cs:250-264`). O valor real em produção é o stopwatch ao vivo `DurationMs` da probe exacta (`M3uTesterService.cs:550,555`). Por isso o preview trata o response time como **desconhecido** e **não** reproduz a ordenação por `ResponseTimeKey` (`ChannelSourceSelector.cs:128,260-261`): em empates nas primeiras quatro chaves de ranking, a ordem e o conjunto seleccionado podem diferir da produção. É uma limitação documentada, **não** uma garantia.
