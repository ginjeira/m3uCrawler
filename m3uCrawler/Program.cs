    ...
    class Program
    {
        static async Task Main(string[] args)
        {
            ...
            if (args.Contains("--telegram"))
            {
                ...
                if (webEnabled)
                {
                    var dashboardTask = WebDashboardService.RunDashboardAsync(outputDir, webPort, importHistoryService, CancellationToken.None);
                    dashboardTask.ContinueWith(t =>
                    {
                ...
                do
                {
                    ...
                    {
                        var searchResult = await scraper.SearchAndTestM3UInTelegram(
                            term,
                            limit: 200,
                            maxConcurrency: 5,
                        ...
                        var workingStreams = searchResult.WorkingStreams;
                        ...
                        await telegramPlaylistManager.SaveToJsonReport(workingStreams, stablePlaylistReportPath);
                        ...
                        await File.WriteAllTextAsync(telegramSearchResultPath, JsonSerializer.Serialize(searchResult, searchOptions), Encoding.UTF8);

                        Console.WriteLine($"\n✨ Arquivos gerados:");
                        Console.WriteLine($"   • Playlist: {playlistPath}");
                        Console.WriteLine($"   • Relatório: {reportPath}");
                        Console.WriteLine($"   • Relatório estável para dashboard: {stablePlaylistReportPath}");
                        Console.WriteLine($"   • Relatório de pesquisa Telegram: {telegramSearchResultPath}");
