...
namespace m3uCrawler.Services
{
    public class TelegramScraperService
    {
        ...
        // Regex mais ampla para capturar playlists e endpoints de stream
        // reais (M3U8, TS, VOD, Xtream /live|movie|series/ e get.php).
        private static readonly Regex _streamUrlRegex = new(
            @"https?://[^\s<>""']+(?:\.m3u8?(?:\?[^\s<>""']*)?|\.ts(?:\?[^\s<>""']*)?|\.mp4(?:\?[^\s<>""']*)?|\.mkv(?:\?[^\s<>""']*)?|\.mov(?:\?[^\s<>""']*)?|/get\.php\?[^/\s<>""']*|/(?:live|movie|series)/[^/\s<>""']*(?:\?[^\s<>""']*)?)",
        ...
        public async Task<TelegramScrapeResult> SearchAndTestM3UInTelegram(
            string keyword, int limit = 200, int maxConcurrency = 5, int maxUrlsToTest = 500, int historyHours = 48)
        {
            ...
            if (foundUrls.Count == 0)
            {
                ...
                var emptyReport = new TelegramSearchReport
                {
                    SearchTerm = keyword,
                    GeneratedAt = DateTime.UtcNow,
                    MessagesMatched = textResults.Count,
                    FoundUrls = 0,
                    TestedUrls = 0,
                    WorkingStreams = 0,
                    FailedStreams = 0,
                    MaxUrlsTested = maxUrlsToTest,
                    XtreamCredentialsDetected = false,
                    XtreamServers = new List<XtreamCredentialInfo>(),
                    SampleFoundUrls = new List<FoundUrlInfo>(),
                    AllFoundUrls = new List<FoundUrlInfo>()
                };

                return new TelegramScrapeResult
                {
                    WorkingStreams = new List<M3uStream>(),
                    AllTestedStreams = new List<M3uStream>(),
            ...
            try
            {
                ...
                var report = new TelegramSearchReport
                {
                    SearchTerm = keyword,
                    GeneratedAt = DateTime.UtcNow,
                    MessagesMatched = textResults.Count,
                    FoundUrls = foundUrls.Count,
                    TestedUrls = allTested.Length,
                    WorkingStreams = working.Count,
                    FailedStreams = failing.Count,
                    MaxUrlsTested = maxUrlsToTest,
                    XtreamCredentialsDetected = xtreamServers.Count > 0,
                    XtreamServers = xtreamServers.Select(kv => new XtreamCredentialInfo
                    {
                        Origin = kv.Key,
                        User = kv.Value.user,
                        Pass = kv.Value.pass,
                        PlaylistUrl = $"{kv.Key}/get.php?username={kv.Value.user}&password={kv.Value.pass}&type=m3u_plus"
                    }).ToList(),
                    SampleFoundUrls = foundUrls.Take(20).Select(f => new FoundUrlInfo
                    {
                        Url = f.Url,
                        ChatTitle = f.ChatTitle,
                        SourceText = f.SourceText,
                        OriginalExtInf = f.OriginalExtInf
                    }).ToList(),
                    AllFoundUrls = foundUrls.Select(f => new FoundUrlInfo
                    {
                        Url = f.Url,
                        ChatTitle = f.ChatTitle,
                        SourceText = f.SourceText,
                ...
                return new TelegramScrapeResult
                {
                    ...
                    AllTestedStreams = allTested.ToList(),
                    SearchReport = report
                };
            }
            finally
                ...
                while (!reachedCutoff)
                {
                    ...
                    foreach (var msgBase in history.Messages)
                    {
                        ...
                        if (normText.Contains(keyword) || normFilename.Contains(keyword))
                        {
                            ...
                            foreach (Match urlMatch in _streamUrlRegex.Matches(text))
                            {
            ...
            List<FoundUrl> foundUrls)
        {
            ...
            foreach (Match urlMatch in _streamUrlRegex.Matches(content))
            {
                if (seenUrls.Add(urlMatch.Value))
                {
